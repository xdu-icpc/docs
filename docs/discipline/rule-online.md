...
代码是使用**比赛开始前**已经编写好并**发布**的，**并未进行自然语言处理，也并未连接网络的**工具生成的。  
使用自然语言处理方法，特别是 (但不限于) 训练产生的语言模型，处理、生成软件源代码等并非自然语言的数据，仍以进行自然语言处理论处。    
对于上述例外情况，选手有义务提供**发布时间**的确凿证据；对于后一种例外情况，选手有义务复现使用工具产生代码的过程。
裁判组有权驳回技术上不够可靠的证据。    
...
取消成绩、禁赛 4 个学期以上，8 个学期以下处罚：  
 ...
 在互联网社交媒体公然对抗违纪调查的；    
 之前有重大违纪记录，再次违纪的。    
特别地，购买代码、文档或使用付费工具生成代码、文档，通过相关题目，超过第 2 条所规定例外范围的，以涉及金钱交易的比赛违纪行为论处。
...
21. 本规则于 2026 年 4 月 14 日修订，
自 2026 年校程序设计竞赛网络赛开始正式实行。如对本规则有任何意见，
